...
if ! oxide ip-pool view --pool default > /dev/null; then
    oxide ip-pool create --name default --description default --ip-version v4
    ...
    oxide ip-pool create --name default-v6 --description default-v6 --ip-version v6
    oxide ip-pool silo link --pool default-v6 --silo $SILO_NAME --is-default true
    oxide ip-pool range add --first fc00:: --last fc00:ffff:: --pool default-v6
fi
