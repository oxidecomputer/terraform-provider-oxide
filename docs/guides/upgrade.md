...
Replace `oxide_subnet_pool_member` with `oxide_system_subnet_pool_member` and
rename `subnet_pool_id` to `pool`. The `pool` attribute accepts a pool name or
ID.
...
+  pool              = oxide_system_subnet_pool.example.id
   subnet            = "192.0.2.0/24"
...
rename `subnet_pool_id` to `pool`, and rename `silo_id` to `silo`. The new
attributes accept names or IDs.
...
state and import the link into the replacement resource using the new format
`SILO_ID/POOL_ID`. Import requires UUIDs even though configuration accepts
names.

```shell
terraform state rm oxide_subnet_pool_silo_link.example
terraform import oxide_system_subnet_pool_silo_link.example 9e199e45-01a6-43d3-8bc3-5b27726e67a6/3e2c6e84-bed8-4c94-afc3-1032082d6a90
```
