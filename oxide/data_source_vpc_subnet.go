...
func (d *vpcSubnetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	subnet, err := d.client.VpcSubnetView(oxideSDK.VpcSubnetViewParams{
		Subnet:  oxideSDK.NameOrId(state.Name.ValueString()),
		Vpc:     oxideSDK.NameOrId(state.VPCName.ValueString()),
		Project: oxideSDK.NameOrId(state.ProjectName.ValueString()),
	})
	if err != nil {
