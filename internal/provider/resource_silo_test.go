...
func TestAccSiloResourceSilo_full(t *testing.T) {
	...
	// Read the silo tls cert from the environment. The cert should be valid for
	// the dns name of the Oxide environment configured by
	// $OXIDE_HOST/$OXIDE_TEST_HOST.
	cert := base64.StdEncoding.EncodeToString([]byte(os.Getenv("OXIDE_SILO_TLS_CERT")))
	key := base64.StdEncoding.EncodeToString([]byte(os.Getenv("OXIDE_SILO_TLS_KEY")))

	config, err := parsedAccConfig(
		resourceSiloConfig{
			BlockName:            blockName,
			SiloName:             siloName,
			TLSCertificateBase64: cert,
			TLSPrivateKeyBase64:  key,
		},
