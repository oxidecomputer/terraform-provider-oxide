...
func TestAccCloudDataSourceProjects_full(t *testing.T) {
	blockName := provider.NewBlockName("datasource-projects")
...
func checkDataSourceProjects(
	...
	return resource.ComposeAggregateTestCheckFunc(
		[]resource.TestCheckFunc{
			...
			resource.TestCheckResourceAttrSet(
				dataName, "projects.0.id",
			),
			resource.TestCheckResourceAttrSet(
				dataName, "projects.0.name",
			),
			resource.TestCheckResourceAttrSet(
				dataName, "projects.0.time_created",
			),
			resource.TestCheckResourceAttrSet(
				dataName, "projects.0.time_modified",
			),
		}...,
	)
}
