...
func TestAccCloudResourceProject_full(t *testing.T) {
	projectName := provider.NewResourceName()
	blockName := provider.NewBlockName("project")
	resourceName := fmt.Sprintf(
		"oxide_project.%s", blockName,
	)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: config,
				Check: checkResourceProject(
					resourceName, projectName,
				),
			},
			{
				Config: configUpdate,
				Check: checkResourceProjectUpdate(
					resourceName, projectNameUpdated,
				),
			},
			{
...
func checkResourceProject(
	resourceName, projectName string,
...
func checkResourceProjectUpdate(
	resourceName, projectName string,
) resource.TestCheckFunc {
	return resource.ComposeAggregateTestCheckFunc(
		[]resource.TestCheckFunc{
...
func testAccProjectDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		return fmt.Errorf(
			"project (%v) still exists", &res.Name,
		)
	}
