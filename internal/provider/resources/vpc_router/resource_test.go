...
func TestAccCloudResourceVPCRouter_full(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: config,
				Check: checkResourceVPCRouter(
					resourceName, routerName,
				),
			},
			{
				Config: configUpdate,
				Check: checkResourceVPCRouterUpdate(
					resourceName, routerNameUpdated,
				),
			},
			{
...
func checkResourceVPCRouter(
	resourceName, routerName string,
	...
	return resource.ComposeAggregateTestCheckFunc(
		[]resource.TestCheckFunc{
			...
			resource.TestCheckResourceAttr(
				resourceName,
				"kind",
				string(oxide.RouterRouteKindCustom),
			),
			resource.TestCheckResourceAttr(
				resourceName, "name", routerName,
			),
			resource.TestCheckResourceAttrSet(
				resourceName, "vpc_id",
			),
...
func checkResourceVPCRouterUpdate(
	resourceName, routerName string,
) resource.TestCheckFunc {
	return resource.ComposeAggregateTestCheckFunc(
		[]resource.TestCheckFunc{
...
func testAccVPCRouterDestroy(
	...
	for _, rs := range s.RootModule().Resources {
		...
		return fmt.Errorf(
			"router (%v) still exists", &res.Name,
		)
	}
