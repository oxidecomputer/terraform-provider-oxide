		...
		Steps: []resource.TestStep{
			{
				...
				Check: checkDataSourceSubnetPool(
					dataSourceName, subnet,
				),
			},
		},
	})
...
func checkDataSourceSubnetPool(
	dataName, subnet string,
) resource.TestCheckFunc {
	return resource.ComposeAggregateTestCheckFunc(
		[]resource.TestCheckFunc{
			resource.TestCheckResourceAttr(
				dataName, "name",
				"terraform-acc-ds-subnet-pool",
			),
			resource.TestCheckResourceAttr(
			...
			resource.TestCheckResourceAttr(
				dataName, "members.0.subnet", subnet,
			),
			resource.TestCheckResourceAttr(
				dataName, "members.0.min_prefix_length",
				"26",
			),
			resource.TestCheckResourceAttr(
				dataName, "members.0.max_prefix_length",
				"28",
			),
		}...)
}
