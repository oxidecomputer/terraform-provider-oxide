...
func testFunctionToVPCFirewallRulesMap_checkSet(resourceName string) resource.TestCheckFunc {
	return resource.ComposeAggregateTestCheckFunc([]resource.TestCheckFunc{
		...
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.1.filters.protocols.0.icmp_code",
			"1-3",
		),
...
func testFunctionToVPCFirewallRulesMap_checkMap(resourceName string) resource.TestCheckFunc {
	return resource.ComposeAggregateTestCheckFunc([]resource.TestCheckFunc{
		...
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.allow-http-https.name",
			"allow-http-https",
		),
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.allow-http-https.description",
			"Allow HTTP and HTTPS.",
		),
		...
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.allow-http-https.filters.protocols.0.type",
			"tcp",
		),
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.allow-http-https.filters.protocols.1.type",
			"udp",
		),
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.allow-http-https.targets.0.type",
			"subnet",
		),
		...
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.icmp.filters.protocols.0.icmp_type",
			"0",
		),
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.icmp.filters.protocols.0.icmp_code",
			"1-3",
		),
		resource.TestCheckResourceAttr(resourceName, "rules.icmp.filters.protocols.1.type", "icmp"),
		resource.TestCheckResourceAttr(
			resourceName,
			"rules.icmp.filters.protocols.1.icmp_type",
			"0",
		),
