...
// ProviderFactories returns the provider factories for
// acceptance tests.
...
// PreCheck verifies the test environment is configured correctly.
...
// NewTestClient creates a new Oxide API client for testing.
func NewTestClient() (*oxide.Client, error) {
	client, err := oxide.NewClient(
		oxide.WithUserAgent(
			fmt.Sprintf(
				"terraform-provider-oxide/%s", Version,
			),
		),
	)
	if err != nil {
...
// ParsedAccConfig parses a template with the given config data
// and returns the resulting string.
func ParsedAccConfig(
	config any, tpl string,
) (string, error) {
...
// NewResourceName generates a unique resource name for testing.
func NewResourceName() string {
	return fmt.Sprintf("acc-terraform-%s", uuid.New())
}

// NewBlockName generates a unique block name for testing.
...
// comparison. Use with resource.TestCheckResourceAttrPtr to
// verify updates don't recreate resources.
...
// VerifyResourceIDChanged verifies the resource ID is different
// from the previously captured ID. Use to confirm a resource was
// replaced (destroyed and recreated).
func VerifyResourceIDChanged(
	...
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == *previousID {
			return fmt.Errorf(
				"resource was not replaced: ID unchanged (%s)",
				*previousID,
			)
		}
		return nil
...
// SiloDNSName returns the silo DNS name for acceptance tests.
...
// NextSubnetCIDR returns sequential /24 subnet CIDRs from the
// 10.128.0.0/16 range.
...
func NextSubnetCIDR(t *testing.T) string {
	...
	if n > 255 {
		t.Fatal(
			"NextSubnetCIDR: exhausted all 256 /24 subnets" +
				" in 10.128.0.0/16",
		)
	}
